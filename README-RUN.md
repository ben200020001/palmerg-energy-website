...
- `RECAPTCHA_SITE_KEY` or `VITE_RECAPTCHA_SITE_KEY`, plus `RECAPTCHA_SECRET_KEY` ([reCAPTCHA v3](https://www.google.com/recaptcha/admin); add `localhost` and your production domain). On Render, `RECAPTCHA_SITE_KEY` is enough at runtime (fetched via `/api/public-config`). If the secret is unset, the server skips verification (local convenience).
